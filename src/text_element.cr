...
class TextElement < Element
  ...
  #
  # ── Complexity ────────────────────────────────────────────────────────────────
  # O(n) calls to R.measure_text, where n is the number of characters in the
  # paragraph.  Each character is measured exactly once (single-char call) and
  # stored in a prefix-sum array.  The break-point search is then pure integer
  # arithmetic — no further R.measure_text calls inside any loop.
  #
  # A naïve linear scan would call R.measure_text O(L) times per wrapped line
  # (each call on a string 1…L characters long), giving O(n²/k) total character
  # work for k lines — quadratic in paragraph length.
  #
  # ── Width formula ─────────────────────────────────────────────────────────────
  # Raylib's MeasureText(text, fontSize) iterates character advance widths and
  # adds a fixed inter-character gap of `fontSize/10` pixels between each
  # adjacent pair (not after the last character):
  #
  #   MeasureText(s[a..b]) = Σ advanceX[i] * scale  +  spacing * (b − a)
  #
  # For the default font at FONT_SIZE 20, scale = fontSize/baseSize = 2 and all
  # advanceX values are integers, so each individual R.measure_text call is also
  # an exact integer and the formula is pixel-perfect.  With the prefix-sum:
  #
  #   seg_width(a, b) = prefix[b+1] − prefix[a]  +  spacing * (b − a)   [O(1)]
  #
  # ── Binary search with interpolation pivot ────────────────────────────────────
  # Goal: find the largest index b such that seg_width(line_start, b) ≤ avail.
  # The invariant is  lo ≤ answer ≤ hi  throughout; ceiling-division mid ensures
  # lo strictly advances on every iteration so the loop always terminates.
  #
  # Before entering the binary search we take one interpolation step:
  #   est = line_start + avail * remaining / full_remaining_width − 1
  # This estimates the answer proportionally.  For uniformly-wide characters
  # (typical prose) it usually lands within 1–2 characters, so the binary search
  # converges in 0–1 additional probes instead of the full log₂(n) steps.
  # All arithmetic is integer to avoid Crystal's Float64|Int32 type-widening.
  #
  # ── Crystal type-inference note ───────────────────────────────────────────────
  # lo, hi, est, and mid participate in a cycle: lo feeds mid, mid feeds lo, and
  # est is clamped and fed back into lo.  Without explicit .to_i32, Crystal's
  # union-type inference widens the whole group to Float64|Int32 (because Int32
  # arithmetic on a Float64|Int32 union promotes to Float64), which then fails
  # when used as an Array index.  The .to_i32 calls on est (post-clamp) and mid
  # anchor the types and break the cycle.
  #
  # ── Word-break search ─────────────────────────────────────────────────────────
  # After finding last_fit (the last character index that fits), we search for
  # the rightmost space in [line_start .. last_fit+1].  The +1 is intentional:
  # if the first overflowing character is itself a space, that space is the
  # natural word boundary — the content before it forms a clean line, and the
  # space is consumed (skipped) when starting the next line.
  private def visual_line_runs : Array({String, Int32})
    avail_i = ([bounds.width - PADDING * 2, 1.0_f32].max).to_i32
    result  = [] of {String, Int32}
    # Inter-character gap: Raylib's DrawText passes fontSize/10 as spacing to
    # MeasureTextEx, which adds it between every adjacent pair of glyphs.
    ...
    text.split('\n').each do |para|
      ...
      # prefix[i] = sum of individual widths for para_chars[0..i-1].
      ...
      while line_start < para_len
        ...
        # Width of every character from line_start to end of paragraph.
        # (remaining − 1) inter-character gaps for remaining characters.
        full_w = prefix[para_len] - prefix[line_start] + spacing * (remaining - 1)

        if full_w <= avail_i
          # Everything remaining fits on one line — no search needed.
          result << {para_chars[line_start, remaining].join, full_offset + line_start}
          break
        end

        # Guard: even a single character exceeds avail — force it to prevent an
        # infinite loop.  Should only happen at extreme zoom-out or tiny bounds.
        ...
        # Invariant: seg_width(line_start, lo) ≤ avail_i
        #            seg_width(line_start, hi+1) > avail_i  (or hi+1 == para_len)
        lo = line_start
        hi = para_len - 1

        # Interpolation pivot: estimate proportionally where the line will break.
        # Uses integer arithmetic throughout to avoid Crystal's Float64|Int32
        # type-widening; see method comment for details.
        ...
        if prefix[est + 1] - prefix[line_start] + spacing * (est - line_start) <= avail_i
          lo = est   # est fits — answer is at or above est
        else
          hi = est - 1  # est overflows — answer is below est
        end

        # Standard binary search on the narrowed range.
        while lo < hi
          # Ceiling division: ensures lo strictly increases when lo == hi − 1,
          # guaranteeing loop termination.  .to_i32 fixes Crystal type inference.
        ...
        # ── Word-break search ────────────────────────────────────────────────
        # Search rightward from last_fit for a space, including last_fit+1 so
        # that a space sitting exactly at the overflow boundary is used as the
        # break point (content before the space fills the line cleanly).
        ...
        if j >= line_start
          # Break at space j: emit content before it, skip the space itself.
          result << {para_chars[line_start, j - line_start].join, full_offset + line_start}
          line_start = j + 1
        else
          # No space in range — hard-break after the last fitting character.
