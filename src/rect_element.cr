...
class RectElement < Element
  ...
  def min_size : {Float32, Float32}
    lines = @label.split('\n')
    max_w = lines.map { |l| @font.measure(l, LABEL_FONT_SIZE) }.max? || 0
  ...
  def char_pos_at_world(world_pos : R::Vector2) : Int32
    ...
    line = lines[vi]
    line_w = @font.measure(line, LABEL_FONT_SIZE)
  ...
  def initialize(@font : Font,
                 ...
                 @label : String = "",
                 id : UUID = UUID.random,
                 @label_color : R::Color = DEFAULT_LABEL_COLOR)
    super(bounds, id)
    init_cursor
  end
end
